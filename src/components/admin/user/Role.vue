       ...
       <!-- ><el-form-item label="功能配置" label-width="120px" prop="perms">
        ...
        <el-form-item label="功能配置" label-width="120px" prop="perms">
          <el-tree
            ...
            :props="prop"
            show-checkbox
            :default-checked-keys="selectedPermsIds"
            node-key="id"
            ref="tree1">
          </el-tree>
        </el-form-item>
  ...
  export default {
    ...
    editRole (role) {
      ...
      for (let i = 0; i < role.perms.length; i++) {
        ...
        for (let j = 0; j < role.perms[i].children.length; j++) {
          permIds.push(role.perms[i].children[j].id)
        }
      }
      ...
      if (this.$refs.tree && this.$refs.tree1) {
        ...
        this.$refs.tree1.setCheckedKeys(permIds)
      }
    },
      onSubmit (role) {
        ...
        this.$axios.put('/admin/role/permission?rid=' + role.id, {
          permIds: this.$refs.tree1.getCheckedKeys()
        }).then(resp => {
          if (resp && resp.data.code === 200) {
